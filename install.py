...
class AutoInstall:
    ...
    def is_installed(self, package):
        ...
        if self.device.shell(f"pm list packages | grep {package}") != "":
            print(self.device.shell(f"pm list packages | grep {package}"))
            return True
        else:
            return False
            ...
            if "English" in labels[0]:
                ...
                if "SM-G955F" in self.device_name:
                    ...
                    if self.is_edit_exist():
                        ...
                        self.element_click(comment="//*[@text='添加语言']/@*")
                        time.sleep(3)
                        self.element_click(comment="//*[@text='English']/@*")
                        time.sleep(3)
                        self.element_click(comment="//*[@text='United States']/@*")
                        time.sleep(3)
                        self.element_click(comment="//*[@text='设为默认']/@*")
                else:
    ...
    def is_edit_exist(self):
        if self.element_is_exist(comment="//*[@text='编辑']/@*", times=2, type=False):
            return False
        else:
            return True
    ...
    def change_eage(self):
        ...
        if "CN" in self.lang or "zh" in self.lang:
            ...
            device_xml = self.get_ui_xml()
            settings = device_xml.xpath("//*[@text='浏览器应用程序']/@*")
            if settings:
                ...
                device_xml = self.get_ui_xml()
                chrome = device_xml.xpath("//*[@text='Chrome']/@*")
                if chrome:
            ...
            self.device.shell('am start -a android.settings.MANAGE_DEFAULT_APPS_SETTINGS')
            device_xml = self.get_ui_xml()
            settings = device_xml.xpath("//*[@text='Browser app']/@*")
            if settings:
                settings_pixel = self.median_value(settings)
                logger.info(f"设备{self.device_id}--找到更换浏览器选项，正在更换浏览器")
                self.device.shell(f"input tap {settings_pixel}")
    ...
    def median_value_pro(self, numer):
        """
        取元素矩阵中间坐标点
        :param num:
        :return:
    ...
    def clear_cache(self):
        ...
        self.device.shell.shell('input keyevent 187')
        try:
            self.element_click(comment="//*[@text='全部关闭']/@*")
        except:
    ...
    def change_vpn(self, data):
        ...
        if self.settings["openvpn"] == "openvpn342":  # 支持版本342
            ...
            time.sleep(3)
            self.element_click(comment="//*[@text='ACCEPT']/@*")  # 点击ACCEPT
            time.sleep(8)
            self.device.input_text(data["vpn_acc"])  # 输入vpn账号
            time.sleep(3)
            self.element_click(comment="//*[@text='Password']/@*")  # 点击密码输入框
            time.sleep(3)
            ...
            try:
                ...
                self.element_click(comment="//*[@text='IMPORT']/@*")  # 点击导入（IMPORT）
            time.sleep(5)
            self.element_click(comment="//*[@resource-id='Edit Profile']/@*")  # 输入vpn密码
            time.sleep(3)
            self.device.input_swipe(start_x=597, start_y=768, end_x=597, end_y=400, duration=300)
            time.sleep(3)
            self.element_click(comment="//*[@resource-id='Enable checkbox Save password']/@*")  # 点击SAVE PASSWORD
            time.sleep(5)
            self.element_click(comment="//*[@resource-id='Text Input']/@*")  # 点击password框
            self.device.input_text(data["vpn_pwd"])
            time.sleep(3)
            self.element_click(comment="//*[@resource-id='save']/@*")  # 点击 ADD PROXY
            time.sleep(3)
            self.element_click(comment="//*[@resource-id='Connection Toggle']/@*")
            time.sleep(3)
            self.element_click(comment="//*[@text='OK']/@*", type=False)
        ...
        #     self.device.input_keyevent(3)  # 返回手机主页（home）
            ...
            if ovpn:
                ...
                print(os.path.join(os.getcwd(), ovpn["path"]))
            ...
            self.device.shell("am start -n net.openvpn.openvpn/net.openvpn.unified.MainActivity t28")
            self.element_click(comment="//*[@text='AGREE']/@*", type=False)  # 点击同意
            time.sleep(3)
            self.element_click(comment="//*[@text='FILE']/@*")
            time.sleep(3)
            self.element_click(comment="//*[@text='ALLOW']/@*", type=False)
            time.sleep(3)
            self.element_click(comment="//*[@text='ALWAYS ALLOW']/@*", type=False)
            time.sleep(3)
            self.element_click(comment=f"//*[@text='{ovpn['file_name']}']/@*", type=True)
            input("")
            time.sleep(3)
            self.element_click(comment="//*[@text='']/@*", type=False)
        else:
    ...
    def eage_ready(self):
        ...
        if "chrome116" in self.settings["chrome"]:
            ...
            self.element_click(comment="//*[@text='ALLOW']/@*", type=False)
            time.sleep(2)
            self.element_click(comment="//*[@text='Accept & continue']/@*", type=False)
            time.sleep(2)
            self.element_click(comment="//*[@text='Next']/@*", type=False)
            time.sleep(2)
            self.element_click(comment="//*[@text='No Thanks']/@*", type=False)
            time.sleep(2)
            self.element_click(comment="//*[@text='ALWAYS ALLOW']/@*", type=False)
            time.sleep(2)
            self.element_click(comment="//*[@text='ALWAYS ALLOW']/@*", type=False)
            if self.element_is_exist(comment="//*[@text='UPDATA']/@*", type=False) == True:
            ...
            if self.element_is_exist(comment="//*[@text='UPDATA']/@*", type=False) == True:
                ...
                self.device.shell("input keyevent KEYCODE_BACK")
                time.sleep(2)
            self.element_click(comment="//*[@text='Continue']/@*", type=False)
            time.sleep(2)
            self.element_click(comment="//*[@text='Use without an account']/@*", type=False)
            time.sleep(2)
            self.device.input_swipe(start_x=489, start_y=1503, end_x=489, end_y=1134, duration=500)
            time.sleep(2)
            self.element_click(comment="//*[@text='Got it']/@*", type=False)
            time.sleep(2)
            self.element_click(comment="//*[@text='Keep Google']/@*", type=False)
    ...
    def test(self):
        ...
        device = client.device(self.device_id)
        self.device.shell("input keyevent KEYCODE_BACK")
